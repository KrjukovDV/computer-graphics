...
materials[2].Color = vec3(0.9, 0.9, 0.9);
...
materials[3].Color = vec3(.0, 1.0, 1.0);
...
materials[4].ReflectionCoef = 1.0;
...
STracingRay mas[100];
int c = 0;
void pushRay(STracingRay ray){mas[c] = ray; c++;}
STracingRay popRay() {return mas[c--];}
bool isEmpty(){return c<0;}

SLight uLight;
SMaterial UMaterials[6];
...
initializeDefaultLightMaterials(uLight, UMaterials);
if (Raytrace(ray, UMaterials, start, final, intersect))
{
...
float start = 0;
float final = 1000000.0;
if (Raytrace(ray, spheres, triangles, UMaterials, start, final, intersect))
{
...
SRay(intersect.Point + reflectDirection * 0.01,
reflectDirection),
